...
# Returns:
#
#      r::ExecutionResults: An ExecutionResults object containing information
#         about the benchmark's full execution history.
...
function execute(
    ...
    if time_used > budget
        return ExecutionResults(false, false, false, s, time_used)
    end
    ...
    if time_used > budget || samples == 1
        return ExecutionResults(true, false, false, s, time_used)
    end
    ...
    if debiased_time_ns > 1_000 * estimate_clock_resolution()
        ...
        f!(s, n_samples, 1)
        return ExecutionResults(true, true, false, s, time() - start_time)
    end
    ...
    return ExecutionResults(true, true, true, s, time() - start_time)
end
